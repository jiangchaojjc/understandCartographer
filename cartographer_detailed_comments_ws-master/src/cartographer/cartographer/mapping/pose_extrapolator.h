...
class PoseExtrapolator : public PoseExtrapolatorInterface {
 public:
  ...
  void AddPose(common::Time time, const transform::Rigid3d& pose) override;  //jc:根据pose对距离和姿态的校准
  ...
  transform::Rigid3d ExtrapolatePose(common::Time time) override; //jc:推测平移和姿态

  ExtrapolationResult ExtrapolatePosesWithGravity( //jc:给定时间的vector返回位姿的vector
      const std::vector<common::Time>& times) override;

  // Returns the current gravity alignment estimate as a rotation from
  // the tracking frame into a gravity aligned frame.
  Eigen::Quaterniond EstimateGravityOrientation(common::Time time) override;  //jc:估计机器人的姿态

 private:
  void UpdateVelocitiesFromPoses();
  void TrimImuData();
  void TrimOdometryData();
  void AdvanceImuTracker(common::Time time, ImuTracker* imu_tracker) const;  //jc:根据imu对姿态进行预测和校准
  ...
  std::deque<TimedPose> timed_pose_queue_;  //保存的扫描匹配之后的位姿
  // 线速度有2种计算途径
  //jc:扫描匹配之后的结果 
  Eigen::Vector3d linear_velocity_from_poses_ = Eigen::Vector3d::Zero(); //用于更新的线速度
  Eigen::Vector3d angular_velocity_from_poses_ = Eigen::Vector3d::Zero(); //用于更新的角速度
  ...
  std::unique_ptr<ImuTracker> imu_tracker_;               // 保存与预测当前姿态    //jc:保存校正位姿
  std::unique_ptr<ImuTracker> odometry_imu_tracker_;      // 用于计算里程计的姿态的ImuTracker //jc:根据里程计数据预测姿态
  std::unique_ptr<ImuTracker> extrapolation_imu_tracker_; // 用于预测姿态的ImuTracker
  TimedPose cached_extrapolated_pose_;

  std::deque<sensor::OdometryData> odometry_data_;
   //jc:通过里程计计算出来的线速度和角速度（不使用imu的情况）
  Eigen::Vector3d linear_velocity_from_odometry_ = Eigen::Vector3d::Zero();
  Eigen::Vector3d angular_velocity_from_odometry_ = Eigen::Vector3d::Zero();//jc:用于不使用imu数据时的角速度的更新
};
