...
void PoseExtrapolator::AddOdometryData(  //jc:在localTrajectoryBuilder421行调用
  ...
  const Eigen::Quaterniond orientation_at_newest_odometry_time =     //jc:orientation_at_newest_odometry_time为一个旋转平移量
      timed_pose_queue_.back().pose.rotation() *
      ExtrapolateRotation(odometry_data_newest.time,
                          odometry_imu_tracker_.get());   //jc:odometry_imu_tracker_位姿 *时间 = 预测的位姿变量 ； 预测的位姿变量 * 之前的位姿的旋转平移量 = 最终预测位姿
  // 将tracking frame的线速度进行旋转, 得到 local 坐标系下 tracking frame 的线速度
  linear_velocity_from_odometry_ =
      orientation_at_newest_odometry_time *
      linear_velocity_in_tracking_frame_at_newest_odometry_time;  //jc:将这个线速度做一个旋转平移到local坐标系下
}

// 预测得到time时刻 tracking frame 在 local 坐标系下的位姿
transform::Rigid3d PoseExtrapolator::ExtrapolatePose(const common::Time time) {   //jc:位姿推测器在初始的时刻是相对于timed_pose_queue_做平移和旋转的推测
  const TimedPose& newest_timed_pose = timed_pose_queue_.back();              //jc:timed_pose_queue_在local_trajectorybuilder_2d.cc   443行初始化位姿推测器时赋值的
