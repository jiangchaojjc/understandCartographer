...
float SlowValueToBoundedFloat(const uint16 value, const uint16 unknown_value,  //jc:又新建了一个转换表，同value_conversion_tables.cc 38行
...
std::unique_ptr<std::vector<float>> PrecomputeValueToBoundedFloat(
  ...
  for (int repeat = 0; repeat != kRepetitionCount; ++repeat) {
    for (int value = 0; value != kValueCount; ++value) {   //jc:将0到32767之间的数按顺序转换到0.1到0.9存到result，存两遍
...
std::unique_ptr<std::vector<float>> PrecomputeValueToCorrespondenceCost() { //jc:将0到32767之间的数按顺序转换到0.1到0.9存到result，存两遍
  return PrecomputeValueToBoundedFloat(                                     //logic:调用本文件41行
...
const std::vector<float>* const kValueToCorrespondenceCost =
    PrecomputeValueToCorrespondenceCost().release();       //logic:调用本文件67行
...
// 将栅格是未知状态与odds状态下, 将更新时的所有可能结果预先计算出来
//jc:CorrespondenceCostToProbability： 相对1-概率； Odds和ProbabilityFromOdds正好相反
std::vector<uint16> ComputeLookupTableToApplyCorrespondenceCostOdds(  //logic:由probability_grid_range_data_inserter_2d.cc 140 行调用
    float odds) {
  std::vector<uint16> result;
  result.reserve(kValueCount); // 32768

  // 当前cell是unknown情况下直接把odds转成value存进来
  result.push_back(CorrespondenceCostToValue(ProbabilityToCorrespondenceCost(  //jc:CorrespondenceCostToValue将概率转到1~32767
  ...
  for (int cell = 1; cell != kValueCount; ++cell) {
    result.push_back(
        CorrespondenceCostToValue(
            ProbabilityToCorrespondenceCost(ProbabilityFromOdds(      //jc:用当前的odds乘以过去？？的odds，再恢复到概率，再取1-概率，再转到对应于1-32767
                odds * Odds(CorrespondenceCostToProbability(    //jc:用当前的odds乘以过去？？的odds来对栅格进行更新
                           (*kValueToCorrespondenceCost)[cell]))))) +                  //jc:kValueToCorrespondenceCost 就是栅格地图空闲的概率的集合，根据索引cell取值
