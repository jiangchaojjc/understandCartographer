...
LocalTrajectoryBuilder2D::TransformToGravityAlignedFrameAndFilter(
    const transform::Rigid3f& transform_to_gravity_aligned_frame,  //jc:将最后一个点云的坐标转换到local坐标系下原点的坐标变换
...
LocalTrajectoryBuilder2D::AddRangeData(      //jc:global_trajectroy_builder.cc 中的81行调用
  ...
  for (const auto& range : synchronized_data.ranges) {
    ...
    range_data_poses.push_back(
        extrapolator_->ExtrapolatePose(time_point).cast<float>()); //jc:range_data_poses为预测出来的trace_frame到local下的坐标变换
  }
  ...
  for (size_t i = 0; i < synchronized_data.ranges.size(); ++i) {
    ...
    const Eigen::Vector3f origin_in_local =
        range_data_poses[i] *
        synchronized_data.origins.at(synchronized_data.ranges[i].origin_index);  //jc: 根据坐标系原点的索引获取原点坐标 然后进行坐标变换  转换到local坐标系下
  ...
  if (num_accumulated_ >= options_.num_accumulated_range_data()) {
    ...
    return AddAccumulatedRangeData(  //jc:调用本文件294行 包含了扫描匹配
        ...
        TransformToGravityAlignedFrameAndFilter(     //jc:range_data_poses.back().inverse()将点云的最后一个坐标变换到local坐标系下原点的坐标变换
            gravity_alignment.cast<float>() * range_data_poses.back().inverse(),  //jc:将点云的最后一个坐标变换到local坐标系下原点的坐标变换，并且姿态为0，初始时刻的坐标点
            accumulated_range_data_), // jc:原点，hit的点云和miss的点云
...
LocalTrajectoryBuilder2D::AddAccumulatedRangeData(   //logic:本文件 271行 AddRangeData调用
  ...
  std::unique_ptr<InsertionResult> insertion_result = InsertIntoSubmap(
      time, range_data_in_local, filtered_gravity_aligned_point_cloud,  //jc:range_data_in_local 匹配后的位姿处的点云
